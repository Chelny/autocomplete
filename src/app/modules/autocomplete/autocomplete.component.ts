import { Component, Input, HostListener, ViewChild, ElementRef, Renderer2, forwardRef } from '@angular/core';
...
export class AutocompleteComponent implements ControlValueAccessor {
  ...
  public validate(control: FormControl): ValidationErrors | null {
    ...
    this.invalidControl = control.dirty && validator(control) !== null;

    return validator(control);
  }

  @HostListener('document:click', ['$event']) public onClick(event: any): void {
    const isControl: boolean = event.target.closest('.control-wrapper') ||
      [...event.target.classList].includes('model');
  ...
  @HostListener('keydown', ['$event']) public onKeydown(event: any): void {
    ...
    const selectItem = (): void => {
      ...
      // Remove focus on input when max selected items is reached else, add focus
      if (this.model.length === this.maxItemsCount) {
        this.controlRef.nativeElement.blur();
      } else {
        this.controlRef.nativeElement.focus();
      }
    };
